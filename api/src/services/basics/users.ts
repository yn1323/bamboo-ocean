...
export const User: UserRelationResolvers = {
  myPokemon: (_obj, { root }) => {
    return db.user.findUnique({ where: { id: root?.id } }).myPokemon()
  },
  myParty: (_obj, { root }) => {
    return db.user.findUnique({ where: { id: root?.id } }).myParty()
  },
  myPartyTag: (_obj, { root }) => {
    return db.user.findUnique({ where: { id: root?.id } }).myPartyTag()
  },
