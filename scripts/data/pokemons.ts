...
export const insertPokemon = async () => {
  ...
  try {
    ...
    const pokemonInserts = pokemons.map(async (pokemon) => {
      if (!pokemon.no) {
    ...
    await retryInsert(pokemonInserts)

    console.log('pokemon Done.', pokemons.length)

    const evolutionInserts = pokemonIdMemo.map(async (p) => {
    ...
    await retryInsert(evolutionInserts)
